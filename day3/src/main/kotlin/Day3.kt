...
private fun part1() {
    ...
    var currentNumber = 0
    var isCurrentNumberAdjacentToSymbol = false
    var sum = 0
    val parts = mutableListOf<Int>()

    val input2d = input.lines().map { it.toCharArray() }

    fun checkForSymbolsNearby(i: Int, j: Int): Boolean {
        for (xOffset in -1..1) {
            for (yOffset in -1..1) {
                if (input2d.getOrNull(i + xOffset)?.getOrNull(j + yOffset)?.isSymbol() == true) {
                    return true
                }
            }
        }
    ...
    fun handleEndOfNumber() {
        if (isCurrentNumberAdjacentToSymbol) {
            println("$currentNumber is part number")
            sum += currentNumber
            parts += currentNumber
        }
    ...
    for (i in input2d.indices) {
        for (j in input2d[i].indices) {
            ...
            if (currentChar.isDigit()) {
                ...
                if (!isCurrentNumberAdjacentToSymbol) {
                    isCurrentNumberAdjacentToSymbol = checkForSymbolsNearby(i, j)
                }
