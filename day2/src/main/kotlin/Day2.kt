...
private fun part1() {
    val illegalRed = ".*(1[3-9]|[2-9]\\d) red.*".toRegex()
    val illegalGreen = ".*(1[4-9]|[2-9]\\d) green.*".toRegex()
    val illegalBlue = ".*(1[5-9]|[2-9]\\d) blue.*".toRegex()

    val result = lines.withIndex().filter {
        !it.value.matches(illegalRed)
                && !it.value.matches(illegalGreen)
                && !it.value.matches(illegalBlue)
    }
    println("Part 1: ${result.sumOf { it.index + 1 }}")
}

private fun part2() {
    val redCount = "(?<count>\\d+) red".toRegex()
    val greenCount = "(?<count>\\d+) green".toRegex()
    val blueCount = "(?<count>\\d+) blue".toRegex()

    fun MatchResult.getCountOrZero() = groups["count"]?.value?.toInt() ?: 0
    fun String.getMax(countRegex: Regex) = countRegex.findAll(this).map { it.getCountOrZero() }.max()

    val result = lines.sumOf { line ->
        val maxRedCount = line.getMax(redCount)
        val maxGreenCount = line.getMax(greenCount)
        val maxBlueCount = line.getMax(blueCount)
        maxRedCount * maxGreenCount * maxBlueCount
    }
    println("Part 2: $result")
}

val lines = """Game 1: 2 blue, 4 green; 7 blue, 1 red, 14 green; 5 blue, 13 green, 1 red; 1 red, 7 blue, 11 green
